...
const DeviceSearchPage = () => {
  ...
  return (
    <div className={`min-h-screen bg-white relative ${isAtBottom ? 'bg-effect-fade-bottom' : ''}`}>
          ...
          {DEVICE_CATEGORIES.map((category) => {
            ...
            return (
              ...
              >
                ...
                <p className="font-body-3-sm 2xl:font-body-2-sm text-black whitespace-nowrap">
                  {category.name}
                </p>
              </button>
            );
      ...
      <div className="max-w-1600 mx-auto px-160 pt-68">
        ...
        <div className="flex items-center gap-0 pt-8">
          ...
          <div ref={priceRef} className="relative flex flex-col gap-16 ml-72">
            ...
            >
              <p
                className={`font-body-1-sm whitespace-nowrap ${
                  selectedPrice ? 'text-blue-600' : showPriceFilter ? 'text-gray-400' : 'text-black'
                }`}
              >
                {selectedPrice
                  ? PRICE_OPTIONS.find((opt) => opt.value === selectedPrice)?.label
                  : '가격대'}
              </p>
                ...
                {PRICE_OPTIONS.map((option, index) => (
                  <button
                    ...
                    className={`flex items-center gap-12 justify-between pb-20 hover:bg-gray-100 transition-colors ${
                      index < PRICE_OPTIONS.length - 1 ? 'border-b border-black' : ''
                    }`}
                  >
          ...
          <div ref={brandRef} className="relative flex flex-col gap-16 ml-32">
            ...
            >
              <p
                className={`font-body-1-sm whitespace-nowrap ${
                  selectedBrand ? 'text-blue-600' : showBrandFilter ? 'text-gray-400' : 'text-black'
                }`}
              >
                {selectedBrand
                  ? BRAND_OPTIONS.find((opt) => opt.value === selectedBrand)?.label
                  : '브랜드'}
              </p>
              <div className="flex items-center justify-center">
                <DropdownIcon
                ...
                {BRAND_OPTIONS.map((option, index) => (
                  <button
                    ...
                    className={`flex items-center gap-10 justify-between pb-10 hover:bg-gray-100 transition-colors ${
                      index < BRAND_OPTIONS.length - 1 ? 'border-b border-black' : ''
                    }`}
                  >
                    <p className="font-body-1-r text-black whitespace-nowrap">{option.label}</p>
      ...
      {selectedProduct && (
        <>
          {/* Background Overlay - GNB(z-50)보다 높게 설정 */}
          <div className="fixed inset-0 bg-black/50 z-[60]" onClick={handleCloseModal} />
